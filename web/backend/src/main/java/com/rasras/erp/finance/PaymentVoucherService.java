...
public class PaymentVoucherService {
        ...
        public PaymentVoucherDto createVoucher(PaymentVoucherDto dto) {
                ...
                try {
                        ...
                        System.err.println("Failed to initiate global approval for voucher: "
                                        + savedVoucher.getVoucherNumber());
                        e.printStackTrace();
                }
        ...
        public PaymentVoucherDto processPayment(Integer voucherId, String paidBy) {
                ...
                if (!"GMApproved".equals(voucher.getApprovalStatus())
                                && !"Approved".equals(voucher.getApprovalStatus())) {
        ...
        private InvoiceComparisonData convertToComparisonData(SupplierInvoice inv) {
                ...
                if (inv.getItems() != null) {
                        for (var invItem : inv.getItems()) {
                                ...
                                if (po != null && po.getItems() != null) {
                                        po.getItems().stream()
                                                        .filter(pi -> pi.getItem().getId()
                                                                        .equals(invItem.getItem().getId()))
                                                        .findFirst()
                                                        .ifPresent(pi -> {
                                                                itemComp.setPoQuantity(pi.getOrderedQty());
                                                                itemComp.setPoUnitPrice(pi.getUnitPrice());
                                                                itemComp.setPoDiscountPercentage(
                                                                                pi.getDiscountPercentage());
                                ...
                                if (grn != null && grn.getItems() != null) {
                                        grn.getItems().stream()
                                                        .filter(gi -> gi.getItem().getId()
                                                                        .equals(invItem.getItem().getId()))
                                                        .findFirst()
                                                        .ifPresent(gi -> {
                                                                itemComp.setGrnQuantity(
                                                                                gi.getAcceptedQty() != null
                                                                                                ? gi.getAcceptedQty()
                                                                                                : gi.getReceivedQty());
                                                                itemComp.setGrnUnitPrice(gi.getUnitCost() != null
                                                                                ? gi.getUnitCost()
                                                                                : (itemComp.getPoUnitPrice() != null
                                                                                                ? itemComp.getPoUnitPrice()
                                                                                                : BigDecimal.ZERO));
                                                                itemComp.setGrnLineTotal(gi.getTotalCost() != null
                                                                                ? gi.getTotalCost()
                                                                                : itemComp.getGrnQuantity().multiply(
                                                                                                itemComp.getGrnUnitPrice()));
                                                        });
                                }
                ...
                if (grn != null && grn.getId() != null) {
                        ...
                        for (PurchaseReturn ret : returns) {
                                returnSubTotal = returnSubTotal
                                                .add(ret.getSubTotal() != null ? ret.getSubTotal() : BigDecimal.ZERO);
                                returnTotal = returnTotal.add(
                                        ...
                                        for (PurchaseReturnItem retItem : ret.getItems()) {
                                                ...
                                                itemComparisons.stream()
                                                                .filter(ic -> ic.getItemName() != null
                                                                                ...
                                                                                && ic.getItemName().equals(retItem
                                                                                                .getItem()
                                                                                                .getItemNameAr()))
                                                                .findFirst()
                                                                .ifPresent(ic -> {
                                                                        BigDecimal existingRetQty = ic
                                                                                        .getReturnedQuantity() != null
                                                                                                        ? ic.getReturnedQuantity()
                                                                                                        : BigDecimal.ZERO;
                                                                        ic.setReturnedQuantity(existingRetQty
                                                                                        .add(retItem.getReturnedQty()));
                                                                        ic.setReturnUnitPrice(retItem.getUnitPrice());
                                                                        BigDecimal existingRetTotal = ic
                                                                                        .getReturnLineTotal() != null
                                                                                                        ? ic.getReturnLineTotal()
                                                                                                        : BigDecimal.ZERO;
                                                                        ic.setReturnLineTotal(existingRetTotal
                                                                                        .add(retItem.getTotalPrice()));
                                                                });
                                        }
                                }
                        }
                ...
                for (InvoiceItemComparison ic : itemComparisons) {
                        BigDecimal retQty = ic.getReturnedQuantity() != null ? ic.getReturnedQuantity()
                                        : BigDecimal.ZERO;
                        BigDecimal netGrnQty = (ic.getGrnQuantity() != null ? ic.getGrnQuantity() : BigDecimal.ZERO)
                                        .subtract(retQty);
                        boolean qtyMatch = ic.getInvoiceQuantity() != null
                                        && ic.getInvoiceQuantity().compareTo(netGrnQty) == 0;
                ...
                BigDecimal grnSubTotal = grn != null
                                ? grnTotal.subtract(
                                                grn.getShippingCost() != null ? grn.getShippingCost() : BigDecimal.ZERO)
                                                .subtract(grn.getOtherCosts() != null ? grn.getOtherCosts()
                                                                : BigDecimal.ZERO)
                                : BigDecimal.ZERO;
                ...
                BigDecimal poDiscountPercentage = po != null && po.getSubTotal() != null
                                && po.getSubTotal().compareTo(BigDecimal.ZERO) > 0
                                                ? po.getDiscountAmount().multiply(new BigDecimal(100)).divide(
                                                                po.getSubTotal(), 2,
                ...
                BigDecimal poTaxPercentage = poAfterDiscount.compareTo(BigDecimal.ZERO) > 0
                                ? poTaxAmount.multiply(new BigDecimal(100)).divide(poAfterDiscount, 2,
                                                RoundingMode.HALF_UP)
                                : BigDecimal.ZERO;

                BigDecimal invDiscountPercentage = inv.getSubTotal() != null
                                && inv.getSubTotal().compareTo(BigDecimal.ZERO) > 0
                                                ? inv.getDiscountAmount().multiply(new BigDecimal(100)).divide(
                                                                inv.getSubTotal(), 2,
                                                                RoundingMode.HALF_UP)
                                                : BigDecimal.ZERO;
                ...
                BigDecimal invTaxPercentage = invAfterDiscount.compareTo(BigDecimal.ZERO) > 0
                                ? inv.getTaxAmount().multiply(new BigDecimal(100)).divide(invAfterDiscount, 2,
                                                RoundingMode.HALF_UP)
                                : BigDecimal.ZERO;
                ...
                return InvoiceComparisonData.builder()
                                ...
                                .grnTaxPercentage(grnSubTotal.compareTo(BigDecimal.ZERO) > 0
                                                ? grnTaxAmount.multiply(new BigDecimal(100)).divide(grnSubTotal, 2,
                                                                RoundingMode.HALF_UP)
                                                : BigDecimal.ZERO)
                                .grnDiscountPercentage(grnSubTotal.compareTo(BigDecimal.ZERO) > 0
                                                ? grnDiscountAmount.multiply(new BigDecimal(100)).divide(grnSubTotal, 2,
                                                                RoundingMode.HALF_UP)
                                                : BigDecimal.ZERO)
                                ...
                                .remainingAmount(inv.getTotalAmount()
                                                .subtract(inv.getPaidAmount() != null ? inv.getPaidAmount()
                                                                : BigDecimal.ZERO))
        ...
        private BigDecimal calculateGrnTotal(GoodsReceiptNote grn) {
                ...
                if (grn.getItems() != null) {
                        subTotal = grn.getItems().stream()
                                        .map(item -> item.getTotalCost() != null ? item.getTotalCost()
                                                        : BigDecimal.ZERO)
                                        .reduce(BigDecimal.ZERO, BigDecimal::add);
                }
