...
public class SupplierQuotationPdfService {

    private final CompanyInfoRepository companyInfoRepository;

    public byte[] generatePdf(SupplierQuotationDto quotation) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            ProcurementPdfStyleSupport.addHeader(
                    ...
                    "تقرير عرض سعر مورد",
                    "رقم العرض",
                    safe(quotation.getQuotationNumber())
            );

            ProcurementPdfStyleSupport.addSectionTitle(document, "بيانات العرض", sectionFont);
            ...
            addLabelValueCell(info, "تاريخ العرض", quotation.getQuotationDate() != null ? quotation.getQuotationDate().toString() : "—", normalFont);
            addLabelValueCell(info, "المورد", safe(quotation.getSupplierNameAr()), normalFont);
            addLabelValueCell(info, "رقم طلب عرض السعر", safe(quotation.getRfqNumber()), normalFont);
            addLabelValueCell(info, "تاريخ الصلاحية", quotation.getValidUntilDate() != null ? quotation.getValidUntilDate().toString() : "—", normalFont);
            addLabelValueCell(info, "الحالة", safe(quotation.getStatus()), normalFont);
            addLabelValueCell(info, "العملة", safe(quotation.getCurrency()), normalFont);
            addLabelValueCell(info, "الإجمالي", formatMoney(quotation.getTotalAmount()), normalFont);
            addLabelValueCell(info, "ملاحظات", safe(quotation.getNotes()), normalFont);
            document.add(info);

            ProcurementPdfStyleSupport.addSectionTitle(document, "بنود العرض", sectionFont);

            PdfPTable items = new PdfPTable(7);
            items.setWidthPercentage(100);
            items.setRunDirection(PdfWriter.RUN_DIRECTION_RTL);
            items.setWidths(new float[]{2.2f, 1f, 1f, 1.2f, 1.2f, 1.2f, 0.7f});
            ...
            addHeaderCell(items, "البوليمر", smallFont);
            addHeaderCell(items, "#", smallFont);

            List<SupplierQuotationItemDto> rows = quotation.getItems();
            if (rows == null || rows.isEmpty()) {
                ...
                for (int i = 0; i < rows.size(); i++) {
                    SupplierQuotationItemDto item = rows.get(i);
                    addDataCell(items, safe(item.getItemNameAr()), smallFont, Element.ALIGN_RIGHT);
                    addDataCell(items, fmt(item.getOfferedQty()), smallFont, Element.ALIGN_CENTER);
                    ...
                    addDataCell(items, safe(item.getPolymerGrade()), smallFont, Element.ALIGN_CENTER);
            ...
            throw new RuntimeException("Failed to generate Supplier Quotation PDF", ex);
        }
    }
