...
const EmployeeDetailPage: React.FC = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                setLoading(true);
                const data = await employeeService.getById(parseInt(id, 10));
                setEmployee(data);
            } catch (e) {
    ...
    const handleEditSuccess = () => {
        if (!id) return;
        employeeService.getById(parseInt(id, 10)).then(setEmployee);
        setIsEditOpen(false);
    };
    ...
    if (!employee) {
        return (
            <div className="text-center py-24">
                <p className="text-slate-500 mb-4">لم يتم العثور على الموظف</p>
                <Link to="/dashboard/employees" className="text-brand-primary font-medium hover:underline">
