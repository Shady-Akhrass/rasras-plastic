...
}> = ({ pr, index, onView, onEdit, onSubmit, onDelete, isExpanded, onToggleExpand, lifecycle, loadingLifecycle }) => (
    <>
        <tr
        ...
        >
            ...
            <td className="px-6 py-4 text-sm text-slate-600">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-brand-primary">{pr.requestedByDeptName || 'المبيعات'}</span>
                </div>
            </td>
            <td className="px-6 py-4">
            ...
            <div className="text-center">
                ...
                <p className="text-slate-500 max-w-md mx-auto">
                    {searchTerm || statusFilter !== 'All'
                        ...
                        : 'لم يتم إنشاء أي طلبات شراء من قسم المبيعات بعد'}
                </p>
            </div>
        </td>
    </tr>
...
const PurchaseRequisitionsListPage: React.FC = () => {
    ...
    const handleViewPR = (id: number) => {
        navigate(`/dashboard/sales/purchase-requisitions/${id}?mode=view`);
    };

    const handleEditPR = (id: number) => {
        navigate(`/dashboard/sales/purchase-requisitions/${id}`);
    };
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || 'فشل حذف طلب الشراء');
        } finally {
            setIsDeleting(false);
        }
    };
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-brand-primary via-brand-primary/95 to-brand-primary/90 
                ...
                <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold mb-2">طلبات الشراء - المبيعات</h1>
                            <p className="text-white/70 text-lg">إدارة ومتابعة طلبات الشراء الخاصة بقسم المبيعات</p>
                        </div>
                    </div>

                    <button
                        onClick={() => navigate('/dashboard/sales/purchase-requisitions/new')}
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="بحث برقم الطلب، اسم الطالب..."
                            value={searchTerm}
...
export default PurchaseRequisitionsListPage;