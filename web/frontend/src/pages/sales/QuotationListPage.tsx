...
const QuotationListPage: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-8 text-white">
                <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                        <div className="p-4 bg-white/10 rounded-2xl"><Tag className="w-10 h-10" /></div>
                        <div>
                            <h1 className="text-2xl font-bold mb-1">عروض الأسعار</h1>
                            <p className="text-white/80">عرض سعر للعميل حسب لائحة الأسعار، صلاحية، شروط دفع</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => navigate('/dashboard/sales/quotations/new')}
                            className="inline-flex items-center gap-2 px-6 py-3 bg-white text-blue-700 rounded-xl font-bold">
                            <Plus className="w-5 h-5" />عرض سعر جديد
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div className="p-4 border-b border-slate-100">
                    <div className="relative max-w-md">
                        ...
                        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
                            placeholder="بحث برقم العرض أو العميل..." className="w-full pr-12 pl-4 py-3 rounded-xl border border-slate-200 focus:border-brand-primary outline-none" />
                    </div>
                </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100">
                                <th className="px-6 py-4 text-right text-xs font-semibold text-slate-600">رقم العرض</th>
                                ...
                                <th className="px-6 py-4 text-right text-xs font-semibold text-slate-600">الصلاحية</th>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-16 text-center text-slate-500">
                                        ...
                                        <button onClick={() => navigate('/dashboard/sales/quotations/new')} className="mt-4 text-brand-primary font-medium">إنشاء عرض سعر</button>
                                    </td>
                                </tr>
                            ) : (
                                paginated.map((q) => (
