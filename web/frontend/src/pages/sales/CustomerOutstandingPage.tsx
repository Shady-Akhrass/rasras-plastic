...
import customerService, { type CustomerOutstanding } from '../../services/customerService';
...
// --- Calculation Helpers ---
const calculateInvoiceTotals = (invoice: SalesInvoiceDto): SalesInvoiceDto => {
    let subTotal = 0;
    ...
    const grandTotal = (subTotal - totalDiscountAmount) + totalTaxAmount + shippingCost;
...
const CustomerOutstandingPage: React.FC = () => {
    ...
    const [summaries, setSummaries] = useState<CustomerOutstanding[]>([]);
    const [orders, setOrders] = useState<SaleOrderDto[]>([]);
    const [allInvoices, setAllInvoices] = useState<SalesInvoiceDto[]>([]);
    ...
    const enrichedSummaries = useMemo(() => {
        return summaries.map(s => {
            // Find all valid invoices for the customer
            const customerInvoices = allInvoices.filter(
                inv => inv.customerId === s.id && (inv.approvalStatus === 'Approved' || !inv.approvalStatus)
            );
            ...
            const totalReturned = s.totalReturned || 0; // Keeping backend value if available
            const currentBalance = totalInvoiced - totalReturned - totalPaid;
    ...
    const filteredSummaries = useMemo(() => {
        const filtered = enrichedSummaries.filter(s => {
            const matchesSearch =
                s.customerNameAr?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                s.customerCode?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    }, [enrichedSummaries, searchTerm, filter]);

    const filteredOrders = useMemo(() => {
        const filtered = orders.filter(o => {
            const isClosed = o.status === 'Completed' || o.status === 'Delivered';
            const matchesSearch =
                o.soNumber?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                o.customerNameAr?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const stats = useMemo(() => {
        ...
        const totalDelivery = calculatedInvoices.reduce(
            (sum, inv) => sum + convertAmount(inv.shippingCost || 0, inv.currency || 'EGP'),
            0
        );
    ...
    const handleViewCustomer = (id: number) => {
        navigate(`/dashboard/crm/customers/${id}`);
    };
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Header */}
            <div className="relative overflow-hidden bg-gradient-to-br from-brand-primary via-brand-primary/95 to-brand-primary/90 rounded-3xl p-8 text-white shadow-lg">
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <p className="text-white/80 text-lg">
                                رصد المدفوعات، الفواتير المستحقة، وحد الائتمان لكل عميل
                            </p>
                        </div>
                    </div>
            ...
            {/* Info Banner */}
            <div className="bg-brand-primary/5 border border-brand-primary/10 p-4 rounded-2xl flex items-center gap-3 text-brand-primary text-sm">
                <AlertCircle className="w-5 h-5 shrink-0" />
                <p className="font-medium">
                    المعادلة الحسابية: الرصيد الحالي = إجمالي الفواتير - إجمالي المرتجعات - إجمالي المدفوعات المستلمة
                </p>
            </div>
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            placeholder={
                                activeTab === 'balances'
                                    ? 'بحث باسم العميل أو كود العميل...'
                                    : 'بحث برقم الأمر أو العميل...'
                            }
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            onFocus={() => setIsSearchFocused(true)}
                            onBlur={() => setIsSearchFocused(false)}
                            className={`w-full pr-12 pl-4 py-3 rounded-xl border-2 transition-all duration-200 outline-none bg-slate-50 focus:bg-white focus:border-brand-primary focus:shadow-lg focus:shadow-brand-primary/10 ${isSearchFocused
                    ...
                    {activeTab === 'balances' && (
                        <div className="flex items-center gap-2 px-4 py-3 bg-slate-50 rounded-xl border-2 border-transparent hover:border-slate-200 transition-all duration-200">
                            ...
                            >
                                ...
                                <option value="Debit">مدين (عليهم مديونية)</option>
                        ...
                        <table className="w-full">
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <EmptyState
                                        icon={DollarSign}
                                        message="لم يتم العثور على عملاء مطابقين للبحث"
                                        columns={7}
                                    />
                                ) : (
                                    filteredSummaries.map((summary, index) => (
                                        <BalanceTableRow
                                            ...
                                            isExpanded={
                                                expandedCustomerId === summary.id
                                            }
                                            onToggleExpand={handleToggleExpand}
                                            invoices={allInvoices.filter(
                                                inv => inv.customerId === summary.id
                                            )}
                                            getCurrencyLabel={getCurrencyLabel}
                                            defaultCurrency={defaultCurrency}
                        ...
                        <table className="w-full">
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <EmptyState
                                        icon={Package}
                                        message="لا توجد أوامر مبيعات مكتملة حالياً"
                                        columns={6}
                                    />
                                ) : (
