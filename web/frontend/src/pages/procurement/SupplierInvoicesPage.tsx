...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
    const config: Record<string, { label: string; bg: string; text: string; border: string; icon: React.ElementType }> = {
        'Pending': { label: 'قيد المراجعة', bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200', icon: Clock },
        'Approved': { label: 'معتمدة', bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', icon: CheckCircle2 },
        'Rejected': { label: 'مرفوضة', bg: 'bg-rose-50', text: 'text-rose-700', border: 'border-rose-200', icon: Ban },
        'Paid': { label: 'مدفوعة', bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-200', icon: Wallet },
        'Unpaid': { label: 'غير مدفوعة', bg: 'bg-slate-50', text: 'text-slate-600', border: 'border-slate-200', icon: DollarSign },
    };
    const c = config[status] || { label: status, bg: 'bg-slate-50', text: 'text-slate-600', border: 'border-slate-200', icon: FileText };
    return (
...
const ApprovalBadge: React.FC<{ status: string }> = ({ status }) => {
    ...
    return <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${c.className}`}>{c.label}</span>;
};

// Tab Button
const TabButton: React.FC<{ active: boolean; onClick: () => void; icon: React.ElementType; label: string; badge?: number; }> = ({ active, onClick, icon: Icon, label, badge }) => (
    <button onClick={onClick} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all duration-300 ${active ? 'bg-brand-primary text-white shadow-lg shadow-brand-primary/30' : 'bg-white text-slate-600 border border-slate-200 hover:border-brand-primary/30 hover:bg-brand-primary/5'}`}>
        <Icon className="w-5 h-5" /> <span>{label}</span>
        {badge !== undefined && badge > 0 && <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${active ? 'bg-white/20 text-white' : 'bg-rose-500 text-white'}`}>{badge}</span>}
    </button>
);
...
const InvoiceRow: React.FC<{
    ...
    const handleDownloadPdf = async (invoice: SupplierInvoiceDto) => {
        try {
            ...
            link.href = url;
            link.setAttribute('download', `invoice-${invoice.supplierInvoiceNo || invoice.id}.pdf`);
            ...
            >
                <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="font-bold text-slate-800 group-hover:text-brand-primary transition-colors">{invoice.invoiceNumber}</div>
                            <div className="text-xs text-slate-400 font-mono">سند: {invoice.supplierInvoiceNo}</div>
                        </div>
                    </div>
                </td>
                <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 bg-slate-100 rounded-lg"><Truck className="w-4 h-4 text-slate-500" /></div>
                        <span className="font-medium text-slate-700">{invoice.supplierNameAr}</span>
                    </div>
                </td>
                <td className="px-6 py-4 text-center">
                    <span className="inline-flex items-center gap-1.5 text-sm text-slate-600"><Calendar className="w-4 h-4 text-slate-400" />{invoice.invoiceDate}</span>
                </td>
                <td className="px-6 py-4 text-center">
                ...
                <td className="px-6 py-4 text-center">
                    ...
                    {invoice.paidAmount! > 0 && <div className="text-xs text-emerald-600 font-bold mt-0.5">مسدد: {formatNumber(invoice.paidAmount)}</div>}
                </td>
                <td className="px-6 py-4">
...
// GRN Row
const GRNRow: React.FC<{ grn: any; index: number; onCreateInvoice: () => void; onViewPO: () => void; }> = ({ grn, index, onCreateInvoice, onViewPO }) => (
    <tr className="group hover:bg-brand-primary/5 transition-colors duration-200 border-b border-slate-100 last:border-0" style={{ animationDelay: `${index * 30}ms`, animation: 'fadeInUp 0.3s ease-out forwards' }}>
        <td className="px-6 py-4">
            <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-emerald-100 to-emerald-50 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform"><Package className="w-5 h-5 text-emerald-600" /></div>
                <div>
                    <div className="font-bold text-slate-800 group-hover:text-brand-primary transition-colors">#{grn.grnNumber}</div>
                    <div className="text-xs text-slate-400 font-mono">{grn.deliveryNoteNo || '---'}</div>
                </div>
            </div>
        </td>
        <td className="px-6 py-4">
            <div className="flex items-center gap-2"><div className="p-1.5 bg-slate-100 rounded-lg"><Truck className="w-4 h-4 text-slate-500" /></div><span className="font-medium text-slate-700">{grn.supplierNameAr}</span></div>
        </td>
        <td className="px-6 py-4 text-center">
            <button
                onClick={onViewPO}
        ...
        <td className="px-6 py-4">
            <button onClick={onCreateInvoice} className="flex items-center gap-2 px-5 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-bold hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40"><DollarSign className="w-4 h-4" /><span>إنشاء فاتورة</span></button>
        </td>
    </tr >
);
...
const SupplierInvoicesPage: React.FC = () => {
    ...
    const fetchInvoices = async () => {
        ...
        } catch (error) { console.error('Failed to fetch invoices:', error); toast.error('فشل تحميل الفواتير'); } finally { setLoading(false); }
    };
    ...
    const fetchPendingGRNs = async () => {
        ...
        } catch (error) { console.error('Failed to fetch pending GRNs:', error); toast.error('فشل تحميل التوريدات المعلقة'); } finally { setLoading(false); }
    };
    ...
    const paginatedPending = filteredPending.slice((currentPage - 1) * pageSize, currentPage * pageSize);
    ...
    const stats = useMemo(() => ({
        ...
        subTotal: processedInvoices.reduce((sum, i) => sum + (i.subTotal || 0), 0),
    ...
    const statusOptions = [{ value: 'All', label: 'جميع الحالات' }, { value: 'Pending', label: 'قيد المراجعة' }, { value: 'Approved', label: 'معتمدة' }, { value: 'Paid', label: 'مدفوعة' }, { value: 'Rejected', label: 'مرفوضة' }];
        ...
        <div className="space-y-6">
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-brand-primary via-brand-primary/95 to-brand-primary/90 rounded-3xl p-8 text-white">
                ...
                <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                        <button onClick={() => navigate(-1)} className="p-3 bg-white/10 backdrop-blur-sm rounded-xl hover:bg-white/20 transition-colors"><ArrowLeft className="w-6 h-6" /></button>
                        <div className="p-4 bg-white/10 backdrop-blur-sm rounded-2xl"><Receipt className="w-10 h-10" /></div>
                        <div><h1 className="text-3xl font-bold mb-2">فواتير الموردين</h1><p className="text-white/70 text-lg">إدارة وتسجيل المطالبات المالية للموردين</p></div>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={() => navigate('/dashboard/procurement/invoices/new')} className="flex items-center gap-2 px-6 py-3.5 bg-white text-brand-primary rounded-xl font-bold hover:bg-white/90 transition-all shadow-lg shadow-black/10 hover:shadow-xl hover:-translate-y-0.5"><Plus className="w-5 h-5" /><span>تسجيل فاتورة</span></button>
                        <button onClick={activeTab === 'invoices' ? fetchInvoices : fetchPendingGRNs} disabled={loading} className="p-3 bg-white/10 backdrop-blur-sm text-white rounded-xl hover:bg-white/20 transition-all duration-200 disabled:opacity-50"><RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} /></button>
                    </div>
                </div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <StatCard icon={Receipt} value={stats.subTotal} label="الإجمالي (قبل الخصم والضريبة)" color="primary" suffix="ج.م" />
                <StatCard icon={Ban} value={stats.discountAmount} label="إجمالي الخصومات" color="rose" suffix="ج.م" />
                <StatCard icon={Clock} value={stats.taxAmount} label="إجمالي الضريبة" color="warning" suffix="ج.م" />
                <StatCard icon={Truck} value={stats.deliveryCost} label="إجمالي مصاريف الشحن" color="blue" suffix="ج.م" />
                <StatCard icon={DollarSign} value={stats.totalAmount} label="الإجمالي النهائي" color="success" suffix="ج.م" />
                <StatCard icon={Package} value={stats.pendingGRNs} label="بانتظار الفوترة" color="purple" onClick={() => setActiveTab('pending')} active={activeTab === 'pending'} />
            </div>

            <div className="flex flex-wrap gap-3">
                <TabButton active={activeTab === 'invoices'} onClick={() => setActiveTab('invoices')} icon={FileText} label="الفواتير المسجلة" />
                <TabButton active={activeTab === 'pending'} onClick={() => setActiveTab('pending')} icon={Truck} label="توريدات بانتظار الفوترة" badge={pendingGRNs.length} />
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1">
                        ...
                        <input type="text" placeholder={activeTab === 'invoices' ? "بحث برقم الفاتورة أو اسم المورد..." : "بحث برقم الاستلام أو المورد..."} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onFocus={() => setIsSearchFocused(true)} onBlur={() => setIsSearchFocused(false)} className={`w-full pr-12 pl-4 py-3 rounded-xl border-2 transition-all duration-200 outline-none bg-slate-50 ${isSearchFocused ? 'border-brand-primary bg-white shadow-lg shadow-brand-primary/10' : 'border-transparent hover:border-slate-200'}`} />
                        {searchTerm && <button onClick={() => setSearchTerm('')} className="absolute left-4 top-1/2 -translate-y-1/2 p-1 hover:bg-slate-100 rounded-full transition-colors"><XCircle className="w-4 h-4 text-slate-400" /></button>}
                    </div>
                    {activeTab === 'invoices' && (
                        <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl border-2 border-transparent hover:border-slate-200 transition-all">
                            ...
                            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-transparent outline-none text-slate-700 font-medium cursor-pointer">{statusOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select>
                        </div>
                    )}
                </div>
            </div>
            ...
            {!loading && (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-600">عرض <span className="font-bold text-slate-800">{activeTab === 'invoices' ? filteredInvoices.length : filteredPending.length}</span> {activeTab === 'invoices' ? 'فاتورة' : 'توريد'}</span>
                </div>
            )}
