...
const SupplierOutstandingPage: React.FC = () => {
    ...
    const fetchSummaries = async () => {
        try {
            setLoading(true);
            const response = await supplierService.getOutstandingSummary();
            setSummaries(response.data || []);
        } catch (error) {
            console.error('Failed to fetch outstanding summaries:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <StatCard icon={AlertCircle} value={stats.providersWithBalance.toString()} label="موردين لهم مديونية" color="amber" />
            </div>
